...
const createUpgrade = (m => /* c8 ignore start */ m.__esModule ? m.default : m /* c8 ignore stop */)(require('./upgrade.js'));

/**
 * @typedef {Object} Options An object with a `document` and zero, one, or more custom namespaces.
 * @property {function} createElement the JSX `createElement` like callback to use internally.
 ...
 * @property {function} jsx
 * @property {HTML} HTML
 * @property {SVG} SVG
 */

const {entries, prototype, setPrototypeOf} = Object;
const {toString, valueOf} = prototype;

/**
 * Given an optional global context, returns a proxy that resolves
 * all tag names into their global constructors + augment upgrades
 * via Custom Elements like lifecycle.
 * @property {Options} [options]
 * @returns {Namespace}
 */
module.exports = (options = W3) => {
  const jsx = (Class, props, ...children) => {
    if (typeof Class === 'function' && Class.tag) {
      let prev;
      const dom = {
        ref(node) {
      ...
      if (props) {
        for (const [key, value] of entries(props)) {
          if (key === 'ref')
            ...
            switch (type) {
              case 'symbol': break;
              // accept only handlers on the DOM
              case 'function':
                if (!key.startsWith('on')) break;
              // accept only non literal objects on the DOM
              case 'object':
                if (
                  type !== 'function' &&
                  value &&
      ...
      return createElement(Class.tag, dom, ...children);
    }
    return createElement(Class, props, ...children);
  };
